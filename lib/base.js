...
Module({
    ...
    getFileView: function (path,renderData) {
        return packetLoader.get("fileview", {request: this.request, response: this.response, path: path,renderData:renderData});
    },
...
Module({
    ...
    render: function (goon) {
        ...
        if (path.indexOf("webapps" + Path.sep + project.getProjectName() + Path.sep + "WEBINF") === -1 && path.indexOf("webapps" + Path.sep + project.getProjectName() + Path.sep + "node_modules") === -1) {
            ...
            if (suffix === "nsp") {
                ths.getModule("nspview", {path: path,data:ths.option.renderData}).doRender(function () {
                    goon();
                });
            } else {
                if (suffix !== "") {
                    fs.stat(path, function (err, data) {
                        if (err) {
                            ...
                            if(path!==global.TopolrServer.getDefaultPagePath("404")) {
                                ...
                                ths.getModule("stringview", {data: "[page can not find,server can not render page.]"}).doRender(function () {
                                    goon();
                                });
                            }
                        } else {
...
Module({
    ...
    render: function (goon) {
        ...
        if(this.option.code==="index"){
            if(request.getProjectURL()==="/"){
                if (project.getProjectConfig().hasPage("index")) {
                    path = project.getProjectConfig().getPagePath("index");
                } else {
                    path = global.TopolrServer.getDefaultPagePath("404");
                }
            }else{
                path=project.getProjectConfig().getBasePath()+request.getProjectURL()+"/index."+TopolrServer.getDefaultSuffix();
            }
        }else{
        ...
        this.getModule("fileview", {path: path,renderData:this.option.data}).doRender(function () {
...
Module({
    ...
    doFilter: function (data, next,end) {
        ...
        if (!doit) {
            ...
            next(this.getModule("fileview", {path: path}));
        }
    }
});
Module({
