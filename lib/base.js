        ...
        if (path.indexOf("webapps" + Path.sep + project.getProjectName() + Path.sep + "WEBINF") === -1 && path.indexOf("webapps" + Path.sep + project.getProjectName() + Path.sep + "node_modules") === -1) {
            ...
            if (suffix === "nsp") {
                ...
                if (suffix !== "") {
                    ...
                    ps.then(function () {
                        var pss=topolr.promise(function (a) {
                            a();
                        });
    ...
    render: function () {
        ...
        if (host) {
            ...
            req.setTimeout(proxy.getTimeout(this.option.hostName), function () {
                ps.then(function () {
                    return ths.getModule("errorview", {data: "proxy timeout"}).render();
                });
            });
            req.on("error", function (e) {
                ps.then(function () {
                    return ths.getModule("errorview", {data: e.stack});
                });
            });
...
Module({
    ...
    doFilter: function (view, next) {
        ...
        view.filter(function (done) {
            var _view = view;
            if (response.getStatusCode() !== "404" && view.info && view.info.fileInfo) {
...
Module({
    ...
    doFilter: function (view, next) {
        var ths = this;
        view.filter(function (done) {
            if (this.info && view && view.typeOf && view.typeOf("fileview")) {
