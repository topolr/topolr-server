...
var manager=require("./../bin/server/manager");
...
Module({
    ...
    processDestroyed:function () {},
    processCreated:function () {},
...
Module({
    ...
    forward: function (option) {
        ...
        if (host) {
            ...
            if(!topolr.is.isEmptyObject(this.getRequest().getMultipartData())){
                var et={};
                for(var i in this.getRequest().getMultipartData()){
                    et[i]=require("fs").createReadStream(this.getRequest().getMultipartData()[i].path);
                }
                for(var i in this.getRequest().getPostData()){
                    et[i]=this.getRequest().getPostData()[i];
                }
                ops.formData=et;
            }else{
                ops.form=this.getRequest().getPostData();
            }
                ...
                if (suffix !== "") {
                    ...
                    ps.then(function () {
                        ...
                        fs.stat(path, function (err, data) {
                            pss.resolve();
                            if (err) {
                                if (path !== manager.getDefaultPagePath("404")) {
                                    pss.then(function () {
                                ...
                                response.setStateData("fileInfo", {
                                    path: path,
                                    suffix: suffix,
                                    mime: manager.getWebConfig().mime[suffix] || "text/html",
                                    fileInfo: data
                                });
                                response.setContentType(manager.getWebConfig().mime[suffix] || "text/html");
...
Module({
    ...
    render: function () {
        ...
        var content = manager.getNspContent(path);
        if (content) {
...
Module({
    ...
    render: function () {
        ...
        if (project.getProjectConfig().hasPage("index")) {
            ...
            path = manager.getDefaultPagePath("index");
        }
    ...
    render: function () {
        ...
        if (this.option.code === "index") {
            if (request.getProjectURL() === "" || request.getProjectURL() === "/") {
                ...
                path = project.getProjectConfig().getBasePath() + request.getProjectURL().split("?")[0] + "/index." + manager.getDefaultSuffix();
            }
        } else {
            if (project.getProjectConfig().hasPage(this.option.code)) {
                path = project.getProjectConfig().getPagePath(this.option.code);
            } else {
                path = manager.getDefaultPagePath(this.option.code);
            }
        }
...
Module({
    ...
    render: function () {
        ...
        var url = manager.getURL() + this.option.request.getURL();
        if (!contenter) {
...
Module({
    ...
    getPort: function (hostname) {
        var a = this.option[hostname];
        return this.option[hostname] ? this.option[hostname].port||"80" : 80;
    }
});
Module({
...
Module({
    ...
    doFilter: function (data, next) {
        ...
        if (response.getStatusCode() !== "404" && response.hasStateData("fileInfo")) {
            var info = response.getStateData("fileInfo"), c = false, sh = topolr.hash.md5(info.path + info.fileInfo.mtime);
