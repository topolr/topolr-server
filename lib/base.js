...
Module({
    ...
    getSteamView: function (steam, suffix) {
        return packetLoader.get("steamview", {
            request: this.request,
            response: this.response,
            steam: steam,
            suffix: suffix
    ...
    forward: function (option) {
        ...
        if (host) {
            ...
            if (!topolr.is.isEmptyObject(this.request.getMultipartData())) {
                var et = {};
                for (var i in this.request.getMultipartData()) {
                    et[i] = require("fs").createReadStream(this.request.getMultipartData()[i].path);
                }
                for (var i in this.request.getPostData()) {
                    et[i] = this.request.getPostData()[i];
                }
                ops.formData = et;
            } else {
                ops.form = this.request.getPostData();
            }
...
Module({
    ...
    render: function () {
        var res = this.option.response;
        res.pipe(this.option.steam);
        res.setContentType(TopolrServer.getWebConfig().mime[this.option.suffix]);
        return topolr.promise(function (a) {
            a();
        });
    }
...
Module({
    ...
    render: function () {
        ...
        if (host) {
            var ps = topolr.promise();
            if (!path) {
                path = this.getRequest().getRawURL();
            } else {
                path = util.url.concatQueryString(path, this.getRequest().getRawURL());
            }
