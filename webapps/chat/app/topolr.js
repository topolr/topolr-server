/*!
 * topolr JavaScript library v1.0.2
 * http://topolr.org/
...
(function () {
    ...
    topolr.app=function (option) {
        if(!app._done&&option) {
            (option.sitePath[option.sitePath.length - 1] !== "/") && (option.sitePath = option.sitePath + "/");
    ...
    var source = {
        ...
        ready:false,
        sourceType: {
            packet: "js-text-content-text",
            template: "html-text-template-text",
            html:"html-text-html-text",
            js: "js-code-code-js",
            css: "css-code-css-css",
            json: "json-text-content-json",
            image: "png-nothing-code-image",
            text: "html-text-content-text"
        },
        current: {
            map: {},
            getPacketPath: function (packetName, type) {
                ...
                if(a) {
                    var _a = packetName.split(/\[|\]/)[0];
                    for (var i in a) {
                        var b = a[i];
                        if (b.indexOf(packetName) !== -1) {
                            var time = 0;
                            for (var m = 0; m < b.length; m++) {
                                var e = b[m];
                                var hash = source.current.map.m[e];
                                var hash2 = source.local[e] ? source.local[e].hash : "";
                                if (hash !== hash2) {
                                    time++;
                                }
                            }
            ...
            get: function (packetName) {
                if(source.current.map.m) {
                    if (source.current.map.m[packetName]) {
                        return source.current.map.m[packetName];
                    } else {
                        return null;
                    }
                }else{
        ...
        has: function (packetName) {
            return source.runable.indexOf(packetName) !== -1;
        },
        get: function (packetName, type) {
            var _m=source.getProcessor(type);
            var loader = _m.loader, trigger = _m.trigger,parser=_m.parser;
            if (source.has(packetName)) {
                return source.trigger[trigger](source.local[packetName].content);
            } else {
                var l = source.local[packetName], isload = true;
                if (l) {
                    var c = source.current.get(packetName);
                    if (!c||l.hash === c) {
                ...
                if (mt.compress) {
                    ...
                    return source.loader.compress(path).then(function (b) {
                        ...
                        for(var i in b){
                            ...
                            for(var h=0;h<ainfo.length;h++) {
                                ...
                                source.local[info.p] = {hash: info.h, content: r};
                            }
                        }
                        source.persister.saveAll(source.local);
                        var l = source.local[packetName];
                        return source.trigger[trigger](l.content,packetName);
                    });
                } else {
                    ...
                    return source.loader[loader](path).then(function (content) {
                        ...
                        if (source.runable.indexOf(packetName) === -1) {
                            source.runable.push(packetName);
                        }
                        source.local[packetName] = {hash: source.current.get(packetName), content: content};
                        source.persister.saveAll(source.local);
        ...
        getPacketPath:function(packetName,type){
            var hash=source.current.get(packetName),r="";
            ...
            this.load(packetName).done(function () {
                ...
                if (re.length === ths.info.length) {
                    ...
                    for (var i = 0; i < ths.info.length; i++) {
                        ...
                        var xcode = packet.deleteR(ths.info[i].code) + "$.is.isEmptyObject(module.exports)?(module.exports=exports):'';";
                        if (d.usestrict === "true") {
                            d.usestrict = true;
                        }
                        if (app.option.debug) {
                            xcode = (d.usestrict?"\"use strict\";":"") + xcode + "//# sourceURL=" + d.path;
                        }
                        try {
                            ...
                            new Function("window", "$", "Module", "Option", "module", "exports", "require","sitePath","App","console", xcode).call(
                                d, window, topolr, topolr.Module, topolr.Option, d, {},
                                function (packetName) {
    ...
    packet.prototype.load = function (pkt) {
        ...
        source.get(pkt,"packet").then(function (content) {
            ...
            for (var i = 0; i < aa.require.length; i++) {
                if (!source.has(aa.require[i].packet)) {
                    queue.add(function (a, b) {
        ...
        for (var i = 0, len = r.replace.length; i < len; i++) {
            ...
            for (var tp = 0, lenp = paths.length; tp < lenp; tp++) {
                ...
                if (q) {
                    ...
                    q.appendChild(template.element(r.replace[i].node));
                }
            }
...
//        console.log(t);
    ...
    servicer.cleanLocalInstance=function (view) {
        for(var i in servicer.localinstance){
            if(servicer.localinstance[i].__creater__===view||servicer.localinstance[i].__creater__.isRemoved()){
                ...
                delete servicer.localinstance[i];
            }
        }
    };
    ...
    servicer.clean = function (view) {
        ...
        for (var i in services) {
            ...
            if (service.typeOf("publicservice")) {
    ...
    module.add({
            ...
            for (var i = 0, len = this._list.length; i < len; i++) {
                ...
                if (view[_type]) {
                    try {
                        ...
                        console.error("[topolr] data callback error name is " + view.type() + " " + e.stack);
                    }
                }
            }
            this._run();
        ...
        _run: function () {
            if (this._queue.length > 0 && this._isrunning === true) {
                ...
                try {
                    this["service_" + task.type](task.data);
                } catch (e) {
                    console.error("[topolr] service action called error name of " + this.type() + " " + e.stack);
                }
            }
        },
        action_get:function(){
    ...
    topolr.Option = function (obj) {
        if (!is.isString(obj)) {
            ...
            if (topolr.___info) {
                obj.name = (topolr.___info.packet ? topolr.___info.packet + "." : "") + obj.name;
            }
            option.add(obj);
        } else {
    ...
    topolr.Module = function (obj) {
        if (topolr.___info) {
            obj.packet = topolr.___info.packet;
        }
        module.add(obj);
    };
